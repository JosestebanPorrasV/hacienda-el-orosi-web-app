    ...
    <>
      ...
      {tools.length !== 0 ? (
        <div className="bg-gray-700 rounded-lg px-4 lg:px-8 py-4 lg:py-6 mt-8 ">
          ...
          >
            <i className="fas fa-tools"></i> {`total: ${tools.length}`}
          </span>
                ...
                <table className="text-center relative w-full">
                  <thead className="bg-gray-600">
                    <tr className="text-lg">
                      <th className="sticky top-0 px-6 py-3" hidden={toolsState !== 'En bodega'}>
                      ...
                      <th className="sticky top-0 px-6 py-3" hidden={toolsState === 'De baja'}>
                        <i className="fas fa-cash-register"></i> Acciones
                      </th>
                    </tr>
                  </thead>
                  <tbody className="text-blue-100 divide-y " style={{ height: '50vh' }}>
                    {results.map((tool) => (
                      <tr key={tool._id}>
                        <th className="px-6 py-4 text-center" hidden={toolsState !== 'En bodega'}>
                          <div className="flex flex-col text-center w-full mt-6 mb-6">
                            <div>
                              <select
                                value="value"
                                onChange={(e) => dispatch(changeStatus(tool._id, e.target.value))}
                        ...
                        <th className="px-6 py-4 text-center">
                          ...
                          <button
                            hidden={tool.status === 'En bodega' || tool.status === 'De baja'}
